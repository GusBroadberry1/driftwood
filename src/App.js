...
Respond with EXACTLY this one section:

## Day 1
Morning / Afternoon / Evening for day one only, matched to their interests. Short bullet points, not paragraphs -- no more than 5 bullet points total, one restaurant tip. Match the depth and length of a single day exactly as it would appear in the full paid itinerary -- not more detailed, not richer, just one real sample day.

Write like a well-travelled friend. Concise, specific. Under 130 words total.`;
      ...
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: "16px", padding: "24px", marginBottom: "20px" }}>
        <OutputSection emoji="📋" title="What You'll Get" last>
          {[
            ...
            "Flight estimate & booking tips",
            "Essential apps for the trip",
            "Language cheat sheet",
            "Packing & prep checklist",
          ].map((item, i, arr) => (
            <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: "10px", padding: "10px 0", borderBottom: i === arr.length - 1 ? "none" : `1px solid ${C.border}` }}>
              <span style={{ color: C.drift, flexShrink: 0 }}>✓</span>
              <span style={{ fontSize: "13.5px", color: C.textMid, fontFamily: font.body }}>{item}</span>
            </div>
          ))}
        </OutputSection>
      </div>
...
{previewResult && renderPreview()}
  ...
  <footer style={{ textAlign: "center", padding: "20px 0 0" }}>
    <div style={{ display: "flex", justifyContent: "center", gap: "16px", marginBottom: "8px" }}>
      <a href="/terms.html" target="_blank" rel="noopener noreferrer" style={{ fontSize: "11px", color: C.muted, fontFamily: font.body, textDecoration: "none" }}>Terms</a>
      <a href="/privacy.html" target="_blank" rel="noopener noreferrer" style={{ fontSize: "11px", color: C.muted, fontFamily: font.body, textDecoration: "none" }}>Privacy</a>
      <a href="/refund.html" target="_blank" rel="noopener noreferrer" style={{ fontSize: "11px", color: C.muted, fontFamily: font.body, textDecoration: "none" }}>Refunds</a>
    </div>
    <p style={{ fontSize: "10px", color: C.muted, fontFamily: font.body, margin: 0 }}>
